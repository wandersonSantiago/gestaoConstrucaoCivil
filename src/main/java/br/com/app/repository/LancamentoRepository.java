...
public interface LancamentoRepository extends JpaRepository<Lancamento,Long>, JpaSpecificationExecutor<Lancamento>, QuerydslPredicateExecutor<Lancamento>{
	...
	@Query("SELECT SUM(valor) FROM Lancamento l where l.tipo = ?1 AND l.dataPagamentoOuRecebimento != null AND l.empreendimento.id = ?2")
	BigDecimal somaSaldoDataInicioEDataFinal(TipoLancamentoEnum tipo, Long idEmpreendimento);
	
	@Query("SELECT SUM(valor) FROM Lancamento l where l.tipo = ?1 AND l.dataVencimento >= ?2 AND l.dataVencimento <= ?3 AND l.dataPagamentoOuRecebimento = null AND l.empreendimento.id = ?4")
	...
	@Query("SELECT SUM(valor) FROM Lancamento l where l.tipo = ?1 AND l.dataVencimento >= ?2 AND l.dataPagamentoOuRecebimento = null AND l.empreendimento.id = ?3")
	...
	@Query("SELECT SUM(valor) FROM Lancamento l where l.empreendimento.id = ?1 AND l.dataVencimento < ?2 AND l.dataPagamentoOuRecebimento = null AND  l.tipo = ?3")
